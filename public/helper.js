...
const createMovieOverview = (overview) => {
    ...
    overviewParagraph.innerHTML = `<h3>Sinopsis: </h3> ${overview}`;
...
const createMovieCast = (cast) => {
    ...
    const castNames = cast.map(actor => actor.name);
    castElement.innerHTML = `<h3>Cast:</h3> ${castNames.join(', ')}`;
    return castElement;
};
...
const createMovieReleaseDate = (date) => {
    ...
    releaseDate.innerHTML = `<h3>Release Date: </h3>${date}`;
    return releaseDate;
};
...
const displayMovie = (movieInfo, castInfo) => {
    ...
    const castInfoElement = createMovieCast(castInfo.cast);
    ...
    movieTextDiv.appendChild(castInfoElement);
